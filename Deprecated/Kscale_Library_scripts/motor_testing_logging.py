...
def monitor_thread():
    ...
    while True:
        ...
        # state = supervisor.get_actuators_state([2,3])
        ...
        state = supervisor.get_actuators_state([2,3,4])
        if state and len(state)==3:
            print("J2:", round(state[0].position), "J3:", round(state[1].position), "J4:", round(state[2].position))
        else:
            if state:
                if len(state) == 1:
