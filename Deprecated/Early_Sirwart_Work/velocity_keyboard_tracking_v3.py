...
import serial
import time
from pynput import keyboard as servo_kb
...
##### end effector serial communication (need to modularize at some point) #####
...
def on_press(key):
    ...
    key_press = False
    if key == servo_kb.Key.space:
        gripper_open = not gripper_open
        cmd = b'G' if gripper_open else b'H'
        key_press = True   

    elif isinstance(key, servo_kb.KeyCode):
        char = key.char
        if char == 'o' or char == 'c':
            cmd = char.encode()
            key_press = True
...
servo_kb_listener  = servo_kb.Listener(on_press=on_press)
servo_kb_listener.daemon = True
servo_kb_listener.start()

#### End of end effector communication
