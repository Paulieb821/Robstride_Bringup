...
def sys_id(pos, torque, time, bandwidth):
    # Setup
    y = pos
    u = torque
    ...
    filtered_y = np.zeros((3, len(y)))
    filtered_u = np.zeros((3, len(u)))
    diff_eq_mat = np.array([[0, 1, 0], [0, 0, 1], [-L0, -L1, -L2]])
    diff_eq_vec = np.array([[0], [0], [L0]])
    filtered_y[:,0] = np.array([y[0], 0, 0])
    ...
    # Set up vectors
    cursive_y = filtered_y[2,:]
    cursive_u = np.vstack((filtered_u[1,:], filtered_u[0,:], -filtered_y[1,:], -filtered_y[0,:]))
    ...
    print("a0: ", round(a0, 2))
    print("\nTransfer function: ")
    print(f"G(s) = {round(b1,2)}*s + {round(b0,2)})/(s^2 + {round(a1,2)}*s + {round(a0,2)})\n")

    G = ctrl.TransferFunction([b1 * torque_amp, b0 * torque_amp], [1, a1, a0])
